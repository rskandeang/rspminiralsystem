...
<?php
			echo $this->Paginator->counter('<label>'.'ចំនួនសរុប'.' <span class="badge badge-success">{:count}</span>&nbsp;'.'  '.'ទំព័រទី' .'&nbsp;<span class="label label-default">{:page}</span>&nbsp;'. 'នៃ'.'&nbsp; <span  class="label label-default">{:pages}</span></label><br/>');
